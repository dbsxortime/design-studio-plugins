...
5. **스크롤 리빌 주의**: `concept.html`처럼 스크롤 등장 애니메이션이 있는 페이지는 열자마자/fullPage 캡처하면 섹션이 비어 보인다 — **끝까지 스크롤한 뒤**(단계 스크롤 후 상단 복귀) 캡처한다.
6. **셀프 점검 후 프레젠테이션**: 사용자에게 보여주기 전에 풀페이지 캡처를 직접 읽어 빈 섹션·잘린 요소·저대비(배경색과 겹쳐 안 보이는 텍스트)를 확인한다. 발견 시 빌더를 고치고 재생성한다(생성물 손수정 금지 — 철칙 ②).
7. **정리**: 캡처가 끝나면 이 서빙 프로세스만 종료한다(스튜디오 `:28572` 프로세스는 절대 건드리지 않는다).
