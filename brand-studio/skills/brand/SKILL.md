...
**물을 것 (자유 서술)**: 브랜드 스토리(무슨 서비스/제품인가), 핵심 타깃, 주고 싶은 인상, **피하고 싶은 인상**, 참고로 좋아하는 브랜드.
**확정 게이트**: 들은 내용을 **"제가 이해한 게 맞나요?"** 3~5줄 요약으로 되짚어 확인받는다. 핵심 키워드를 **정확히 3개**로 압축한다(스키마 요구).
**brand.json 기록**: `meta.project`, `brief.keywords`(정확히 3개), `brief.story`/`brief.audience`/`brief.avoid`, `progress.phase = "brief"`. → 승인 시 다음 미팅.
